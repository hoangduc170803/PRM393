...
class LlamaService {
  ...
  Future<String> _generateTextInternal({
    ...
    final nTokens = bindings.llama_tokenize(
      ...
      tokens,
      maxPromptTokens,
      false, // add_special: MUST BE FALSE for ChatML/instruct models because we provide explicit tags
    ...
    // Build a sane default sampler chain for JSON extraction
    bindings.llama_sampler_chain_add(
      sampler,
      bindings.llama_sampler_init_temp(0.0), // temp=0 guarantees deterministic/greedy sampling
    );
    bindings.llama_sampler_chain_add(
      sampler,
      bindings.llama_sampler_init_dist(0), // dist is required to actually select a token
    );
    ...
    for (int i = 0; i < maxTokens; i++) {
      ...
      if (i < 10) {
        print('Debug - Generated token at $i: $newToken');
      }
  ...
  await for (final dynamic msg in receivePort) {
    ...
    try {
      ...
      for (int i = 0; i < maxTokens; i++) {
        ...
        if (newToken == bindings.llama_token_eos(vocab)) {
          sendStatus(requestId, 'EOS token reached at position $i');
          break;
        }

        if (i < 10) {
          sendStatus(requestId, 'Debug - Generated token at $i: $newToken');
        }
