...
🧠 How the AI Works
Image Capture: The user picks a receipt image from the gallery (or takes a photo).
Preprocessing & OCR: The image is preprocessed and read by Tesseract OCR to extract raw text.
Local Text Generation: The text is passed into a highly quantized Llama model format (.gguf) running entirely within a separate isolate thread so the main UI never freezes.
Greedy JSON Extraction: The model has been strictly prompted with greedy deterministic sampling to output only a valid JSON structure representing your receipt's metadata.
Storage: Data is parsed and saved securely to your local SQLite Database.
🤝 Contributing
